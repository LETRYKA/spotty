...
import UnfAlert from "./../../_web/_components/unfAlert";
interface Friend {
  id: string;
  name: string;
  email: string;
    ...
    <div className="flex flex-col gap-6">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {pendingRequests.length === 1 && (
              <p className="text-white/70">{pendingRequests[0]?.name}</p>
            )}
            {pendingRequests.length > 1 && (
              <div>
                <p className="text-xs text-white/70">
                  {pendingRequests[0]?.name} + {pendingRequests.length - 1}{" "}
                  others
                </p>
              </div>
            )}
      ...
      {friends.map((friend) => (
        <div key={friend.id} className="flex justify-between items-center">
          ...
          <UnfAlert userId={userId as string} friendId={friend.id as string} />
        </div>
      ))}
    </div>
  );
